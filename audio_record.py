...
class AudioRecord(object):
  ...
  def __init__(self, channels, sampling_rate: int) -> None:
    ...
    self._audio_buffer = []
    self._desired_sample_rate = sampling_rate
    self._samplerate = sd.query_devices()[0]['default_samplerate']
  ...
  def buffer(self) -> np.ndarray:
    ...
    if self._audio_buffer:
      b = np.concatenate(self._audio_buffer)
      converter = 'sinc_fastest'  # or 'sinc_fastest', ...
      ratio = self._desired_sample_rate / self._samplerate 
      result = samplerate.resample(b, ratio, converter)
